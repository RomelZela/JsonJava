            ...
            private void showDocumentLines(ArrayList<String> textLines) {
                ...
                if (textLines.size() > 0){
                    ...
                    for (int index = 0; index < textLines.size(); index++) {
                    	JSONArray arrayListJson = new JSONArray();
                    	  JSONObject listJson = new JSONObject();
                    	  
                    	arrayListJson.put("doc");
                    	listJson.put("line", index);
                    	listJson.put("text",textLines.get(index));
                    	
                    	  arrayListJson.put(listJson);
                    	  
                    	  printLnToConsole(new Gson().toJson(arrayListJson));
                    }
